...
namespace ErrorCorrectionDemo {
    ...
    operation ErrorCorrectionExperiment() : Result {
        ...
        // Step 1: Encode logical |0⟩ as |000⟩ (default)
        ...
        // Step 4: Measure corrected logical qubit
        let result = M(qs[0]);
    ...
    operation ShorExperiment() : Result {
        ...
        // Step 1: Encode Shor logical |0⟩
        EncodeShor(qs);

        // Step 2: Inject a single bit-flip or phase-flip
        Z(qs[4]); // Test with a phase flip on qubit 4

        // Step 3: Decode manually (or skip to logical measurement)
        // NOTE: Full Shor decoding not implemented here — advanced session

        // Step 4: Measure one qubit from each block
        let r0 = M(qs[0]);
        let r1 = M(qs[3]);
        let r2 = M(qs[6]);
